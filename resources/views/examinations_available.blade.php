...
@include('includes2/modal_examinations')
...
<body class="hold-transition sidebar-mini layout-fixed">
                                        ...
                                        <tbody>
                                            @foreach ($examinations as $examination)
                                                <tr>
                                                    ...
                                                    <td><a
                                                            href="{{ route('downloadReviewers', ['examination_id' => $examination['id']]) }}">Download
                                                            Reviewers</a></td>
                                                    <td>

                                                        @php
                                                            $currentDateTime = now()->setTimezone('Asia/Manila'); // Get the current date and time in the 'Asia/Manila' timezone
                                                            $examDateTime = \Carbon\Carbon::parse(
                                                                $examination['examination_at'],
                                                                'Asia/Manila',
                                                            ); // Parse examination_at string as a Carbon object with the correct timezone
                                                            $oneHourAfterExam = $examDateTime->copy()->addHour(); // Add 1 hour to the exam time
                                                        @endphp


                                                        @if ($currentDateTime >= $examDateTime && $currentDateTime <= $oneHourAfterExam)
                                                        ...
                                                        {{-- <button type="button" class="btn btn-success btn-sm take"
                                                            data-id="{{ $examination['id'] }}"><i
                                                                class="fas fa-pen"></i>
                                                            Take Examination</button> --}}

                                                        {{-- <a href="/portal/examination/attempt/{{ $examination['id'] }}"
                                                            class="btn bg-success btn-sm" name = "examination_id"><i
                                                                class="fas fa-pen"></i> Take Examination</a> --}}
    ...
    <script>
        $(function() {
            ...
            $('#example2 tbody').on("click", ".take", function() {
                $('#take').modal('show');
                var id = $(this).data('id');
